<template>
  <ArrPanel class="app-option-details" title="应用详情" border>
    <div class="app-details-container" v-if="currentApp">
      ...
      <EntityList :globalComponents="currentApp.globalComponents" :entities="currentApp.entities" :onChange="(value) => handleDataChanged('entities', value)" />
      <el-button-group class="float-button">
        <el-button type="primary" @click="handleSaveOpen">保存</el-button>
        <el-button type="danger" @click="handleRestartTheServer">重启</el-button>
      </el-button-group>
      ...
      <el-dialog
        ...
        <el-alert title="您的配置信息如下，请仔细检查是否有配置错误，保存如有错误，请不要提交 arrplus.config.json 文件！" type="warning" effect="dark" :closable="false"></el-alert>
        <pre class="code">{{currentApp}}</pre>
